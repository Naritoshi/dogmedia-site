...
function onFormSubmit(e) {
  ...
  // スクリプトプロパティ 'ALLOWED_EMAIL' に自分のGmailアドレスを設定してください
...
function processFormImage(file, location, category, memo, props) {
  ...
  // Gemini API 設定 (Gemini 1.5 Flash を使用)
  ...
  // 1. 画像をGitHubへアップロード
  uploadToGitHub(repo, githubToken, imagePath, base64Image, `Add image: ${baseName}`);

  // 2. Markdownを作成してアップロード
...
`;
  ...
  uploadToGitHub(repo, githubToken, postPath, markdownBase64, `Add post: ${articleData.title}`);
  
  Logger.log(`✅ 投稿完了: ${articleData.title}`);
}

/**
 * GitHub APIを使ってファイルを作成/更新する
 */
function uploadToGitHub(repo, token, path, contentBase64, message) {
